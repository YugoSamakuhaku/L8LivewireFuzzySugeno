...
<div>
    @if (filled($booleanFilter))
        <div @class([
            ...
            <select id="input_boolean_filter_{{ data_get($booleanFilter, 'field') }}" style="{{ data_get($column, 'headerStyle') }}" class="power_grid {{ $theme->selectClass }} {{ $class }} {{ data_get($column, 'headerClass') }}" wire:input.lazy="filterBoolean('{{ $booleanFilter['dataField'] }}', $event.target.value, '{{ $booleanFilter['label'] }}')"
