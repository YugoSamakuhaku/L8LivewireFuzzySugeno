...
<div>
    ...
    @if (filled($inputText))
        <div class="{{ $theme->baseClass }}" style="{{ $theme->baseStyle }}">
            <div class="relative">
                <select class="power_grid {{ $theme->selectClass }} {{ data_get($column, 'headerClass') }}" style="{{ data_get($column, 'headerStyle') }}" wire:model.defer="filters.input_text_options.{{ $field }}" wire:input.defer="filterInputTextOptions('{{ $field }}', $event.target.value, '{{ data_get($inputText, 'label') }}')">
