    ...
    public function run(): void
    {
        ...
        foreach ($getAllPurchase as $purchase) {
            InggridientHistory::updateOrCreate(
                [
                    'id_inggridient' => $purchase->id_inggridient, 'date' => Carbon::create(Carbon::create($purchase->date_purchase)->format('Y'), Carbon::create($purchase->date_purchase)->format('n'), '01')
                ],
        ...
        foreach ($getAllSale as $sale) {
            InggridientHistory::updateOrCreate(
                [
                    'id_inggridient' => $sale->id_inggridient, 'date' => Carbon::create(Carbon::create($sale->date_sale)->format('Y'), Carbon::create($sale->date_sale)->format('n'), '01')
                ],
            )->increment('stock_out', $sale->total_usage_amount);
        }

        $inggridientHistory = InggridientHistory::get();

        foreach ($inggridientHistory as $key_history => $history) {
            if (!isset($inggridientHistory[$key_history - 1]) ? true : false) {
                $history->stock = 0;
                ...
                if($inggridientHistory[$key_history - 1]->id_inggridient == $history->id_inggridient){
                    $stock = $inggridientHistory[$key_history - 1]->last_stock;
                }else{
                    $stock = 0;
                }
